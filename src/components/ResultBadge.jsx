  ...
  return (
    ...
    >
      <div className="bg-white border-2 border-[#E8DDC7] rounded-2xl shadow-sm p-5 sm:p-8">
        ...
        >
          ...
          <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
            <div className="bg-white px-5 py-3 rounded-xl border border-current/20 shadow-2xs flex items-center gap-3">
              <Award className="w-6 h-6 text-[#C9962F]" aria-hidden="true" />
        ...
        {/* Rhythm & Timing Breakdown (Only for Layer 1: Haptic Rhythm) */}
        {challengeType === 'haptic' && details && details.intervalErrors && (
          <div className="mt-6 border border-[#E8DDC7] rounded-xl p-4 sm:p-5 bg-white">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-xs font-bold uppercase tracking-wider text-[#3E1220] flex items-center gap-1.5">
                <Clock className="w-4 h-4 text-[#C9962F]" />
                Layer 1: Rhythm Interval Breakdown
              </h3>
            ...
            <div className="space-y-2">
              {details.intervalErrors.map((err, idx) => {
                  ...
                  <div
                    ...
                    className={`flex flex-col sm:flex-row sm:items-center justify-between p-2.5 rounded-lg text-xs font-mono border transition-all ${
                      isOutlier
                        ...
                        : 'bg-[#FAF6EE]/70 border-[#E8DDC7]/60 text-stone-800'
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-stone-700">Interval #{idx + 1}</span>
                      <span className="text-stone-600 text-[11px]">
                        Target: <strong>{err.expected}ms</strong> • Recorded:{' '}
                        <strong>{err.actual}ms</strong>
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5 mt-1 sm:mt-0">
                      <span
                        className={`font-bold px-2 py-0.5 rounded-md text-[11px] inline-flex items-center gap-1 ${
                          isOutlier
                            ? 'bg-red-100 text-red-800 border border-red-300'
                            : isElevated
                            ? 'bg-amber-100 text-amber-900 border border-amber-300'
                            : 'bg-[#EFF6EC] text-[#27500A] border border-[#27500A]/20'
                        }`}
                      >
                        {isOutlier && <AlertTriangle className="w-3 h-3 text-red-600 shrink-0" />}
                        ±{err.percentage}% dev
                        {isOutlier ? ' • Exceeded 60% Cap' : isElevated ? ' • Elevated' : ' • Normal'}
                      </span>
                    </div>
                  </div>
                );
            ...
            {/* Timing metrics & Outlier Guard Breakdown */}
            <div className="mt-4 pt-3 border-t border-[#E8DDC7]">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-center mb-3">
                <div className="bg-[#FAF6EE]/70 p-2 rounded-xl border border-[#E8DDC7]/60">
                  <span className="text-[10px] uppercase font-bold text-stone-500 block tracking-wider">
                    Mean Error
                  </span>
                  <span className="font-mono font-black text-sm text-[#3E1220] block mt-0.5">
                    ±{details.meanErrorPercentage ?? 0}%
                  </span>
                  <span className="text-[10px] text-stone-500 font-medium">45% weight</span>
                </div>
                ...
                <div className="bg-[#FAF6EE]/70 p-2 rounded-xl border border-[#E8DDC7]/60">
                  ...
                  <span
                    className={`font-mono font-black text-sm block mt-0.5 ${
                      details.outlierExceeded ? 'text-red-700' : 'text-[#3E1220]'
                    }`}
                  >
                    ±{details.maxErrorPercentage ?? 0}%
                  </span>
                  <span className="text-[10px] text-stone-500 font-medium">55% weight</span>
                </div>
                ...
                <div className="bg-[#FAF6EE]/70 p-2 rounded-xl border border-[#E8DDC7]/60">
                  ...
                  <span className="font-mono font-black text-sm text-[#27500A] block mt-0.5">
                    60% limit
                  </span>
                  <span className="text-[10px] text-stone-500 font-medium">Strict ceiling</span>
                </div>
                ...
                <div className="bg-[#FAF6EE]/70 p-2 rounded-xl border border-[#E8DDC7]/60">
                  ...
                  <span
                    className={`font-mono font-black text-xs block mt-1 ${
                      details.outlierExceeded ? 'text-red-700' : 'text-[#27500A]'
                    }`}
                  >
                    {details.outlierExceeded ? 'Disqualified' : 'Within Limit'}
                  </span>
                  <span className="text-[10px] text-stone-500 font-medium">
                    {details.outlierExceeded ? 'Capped <58%' : 'Passed'}
                  </span>
                </div>
              </div>

              {/* Justification note for judges */}
              <div className="p-2.5 rounded-lg bg-[#FAF6EE]/90 border border-[#E8DDC7] text-[11px] text-stone-700 leading-relaxed flex items-start gap-2">
                <Info className="w-4 h-4 text-[#C9962F] shrink-0 mt-0.5" />
                <div>
                  <span className="font-bold text-[#3E1220]">Scoring Formula & Outlier Policy:</span>{' '}
                  Confidence applies dual weighting (45% mean deviation + 55% peak outlier deviation). Any individual interval deviating by ≥60% triggers an Outlier Cap, capping the score strictly below the 65% pass threshold so erratic taps cannot pass even if other taps are accurate.
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Action Controls */}
        <div className="mt-8 pt-6 border-t border-[#E8DDC7] flex flex-wrap items-center justify-between gap-3">
          <button
            type="button"
            onClick={onReset}
            className="px-4 py-2.5 rounded-xl border border-[#3E1220] text-[#3E1220] text-xs font-bold hover:bg-[#3E1220]/5 flex items-center gap-2 cursor-pointer"
          >
